...
namespace MovieService
{
    public class LoginRegister : Controller
    {
        ...
        public async Task<ActionResult<UserModel>> Register([FromBody] RegisterDataTransferObject request)
        {
            ...
            cmd.Connection = conn;
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "INSERT INTO Users(user_guid, user_name, password_hash, password_salt, email, user_role) VALUES ('" + Guid.NewGuid().ToString() + "', '" + user.userName + "', '" + Convert.ToBase64String(user.passwordHash) + "', '" + Convert.ToBase64String(user.passwordSalt) + "', '" + user.email + "', 'Normal')";
            try
            {
                NpgsqlDataReader reader = cmd.ExecuteReader();
            }
